...
import { calcTrade, formatNumber, parseAmount } from '@/lib/positionSize';
    ...
    return (
        <label className="block">
            <span className="flex items-baseline gap-1.5 mb-1.5">
                ...
                {optional && <span className="text-[10.5px] text-muted-foreground/60">необязательно</span>}
            </span>
 ...
 * Калькулятор сделки.
 *
 * Один расчёт на все рынки: вход, стоп и допустимый риск задают объём
 * везде одинаково. Разделения на бинарки, форекс и крипту нет
 * намеренно - оно заставляло выбирать рынок до того, как человек
 * ввёл хоть одно число, а сама формула от рынка не зависит. Объём
 * выдаётся сразу в единицах, лотах и деньгах, и каждый берёт то, чем
 * измеряет свой рынок.
 *
...
export function PositionCalculator({ open, onClose }: PositionCalculatorProps) {
    ...
    }), [deposit, risk, entry, stop, target, leverage]);
        ...
        >
            ...
            {result ? (
                ...
                >
                    ...
                    <Row label="Объём в единицах" value={formatNumber(result.units, 4)} />
                    <Row label="В лотах" value={formatNumber(result.lots, 2)} />
                    <Row label="Объём в деньгах" value={`${formatNumber(result.notional)} $`} />

                    {parseAmount(leverage) > 1 && (
                        <>
                ...
                <div className={cn(PANEL, 'p-5 text-center text-[13px] text-muted-foreground')} style={PANEL_BG}>
                    Введите депозит, риск, цену входа и стоп - остальное посчитается
                </div>
            )}
            ...
            {result !== null && parseAmount(leverage) > 1 && (
